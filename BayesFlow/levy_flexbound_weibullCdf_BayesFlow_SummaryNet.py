...
#     print("n_samples", n_samples)
    ...
    for i in range(batch_size):
        out = cds.levy_flexbound(v[i], 
                                ...
                                boundary_params = {"alpha": alpha[i], "beta":beta[i]})
    ...
    param = np.concatenate((a.reshape(-1, 1), v.reshape(-1, 1), w.reshape(-1, 1), alpha.reshape(-1, 1), beta.reshape(-1, 1)), axis=1)
...
def load_model_and_opt(n_inv_blocks, summary_dim, global_step, inference=False):
    ...
    manager = tf.train.CheckpointManager(checkpoint, './checkpoints_boundParams/levy_flexbound_weibul_summarynet_{}'.format(n_inv_blocks), max_to_keep=2)
    if inference:
