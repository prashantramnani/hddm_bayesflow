...
wandb.init(project="bayes_flow-summary64", sync_tensorboard=True)
#wandb.init(project="bayes_flow-summary64", resume=True, id='3ncih2ky')
...
#     print("n_samples", n_samples)
    ...
    param = np.concatenate((v.reshape(-1, 1), a.reshape(-1, 1), w.reshape(-1, 1), theta.reshape(-1, 1)), axis=1)
...
def load_model_and_opt(n_inv_blocks, summary_dim, global_step, inference=False):
    ...
    manager = tf.train.CheckpointManager(checkpoint, './checkpoints/ddm_flexbound_angle_summarynet_dim64_wandb{}'.format(n_inv_blocks), max_to_keep=2)
    if inference:
