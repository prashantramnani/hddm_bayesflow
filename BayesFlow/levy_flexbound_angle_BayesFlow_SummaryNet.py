...
def data_generator_levy_flexbound(batch_size, n_samples):
...
#     print("n_samples", n_samples)
    ...
    param = np.concatenate((a.reshape(-1, 1), v.reshape(-1, 1), w.reshape(-1, 1), theta.reshape(-1, 1)), axis=1)
...
def load_model_and_opt(n_inv_blocks, summary_dim, global_step, inference=False):
    ...
    manager = tf.train.CheckpointManager(checkpoint, './checkpoints_boundParams/levy_flexbound_angle_summarynet_{}'.format(n_inv_blocks), max_to_keep=2)
    if inference:
...
def train_model(n_inv_blocks, summary_dim, num_threads):
    ...
    for ep in range(1+epoch_restore, epochs+1):
        # with tqdm(total=iterations_per_epoch, desc='Training epoch {}'.format(ep)) as p_bar:
            ...
            # Run training loop
        losses = train_online_ml(model, optimizer, data_generator_levy_flexbound, iterations_per_epoch, 
