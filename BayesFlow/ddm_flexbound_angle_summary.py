...
wandb.init(project="bayesflow_angle_summarydim_100-5000", sync_tensorboard=True)
...
def data_generator_ddm_flexbound(batch_size, n_samples):
    ...
    ndt = np.random.uniform(0.0, 2.0, batch_size)
    theta = np.random.uniform(0, (np.pi/2)- 0.1, batch_size)
...
#     print("n_samples", n_samples)
    ...
    param = np.concatenate((v.reshape(-1, 1), a.reshape(-1, 1), w.reshape(-1, 1), ndt.reshape(-1, 1), theta.reshape(-1, 1)), axis=1)
...
def load_model_and_opt(n_inv_blocks, summary_dim, global_step, inference=False):
    ...
    manager = tf.train.CheckpointManager(checkpoint, './checkpoints/ddm_flexbound_angle_summarynet_dim{}_100-5000_wandb_{}'.format(summary_dim ,n_inv_blocks), max_to_keep=2)
    if inference:
...
def train_model(n_inv_blocks, summary_dim, num_threads):
    ...
    for ep in range(1+epoch_restore, epochs+1):
        ...
        # Run training loop
        losses = train_online_ml(model, optimizer, data_generator_ddm_flexbound, iterations_per_epoch, 
                        batch_size, num_threads=num_threads, p_bar=None, clip_value=clip_value, global_step=global_step, 
                        transform=None, n_smooth=100)
        
        if ep > 2:
            wandb.log({'epoch': ep-1, 'loss': np.mean(losses['ml_loss'])})
        print("LOSS: ", losses['ml_loss'])
        manager.save()
