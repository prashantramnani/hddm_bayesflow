...
def data_generator_ddm_flexbound(batch_size, n_samples):
    v = np.random.uniform(-2, 2, batch_size)
    a = np.random.uniform(0.3, 2, batch_size)
    w = np.random.uniform(0.2, 0.7, batch_size)
    dw = np.random.uniform(0.0, 0.4, batch_size)
    sdv = np.random.uniform(0.0, 0.5, batch_size)
    dndt = np.random.uniform(0.0, 0.25, batch_size)
...
#     print("n_samples", n_samples)
    ...
    param = np.concatenate((a.reshape(-1, 1), v.reshape(-1, 1), w.reshape(-1, 1), dw.reshape(-1, 1), sdv.reshape(-1, 1), dndt.reshape(-1, 1)), axis=1)
...
def load_model_and_opt(n_inv_blocks, summary_dim, global_step, inference=False):
    ...
    manager = tf.train.CheckpointManager(checkpoint, './checkpoints_boundParams/full_ddm_constant_summarynet_{}'.format(n_inv_blocks), max_to_keep=2)
    if inference:
...
def train_model(n_inv_blocks, summary_dim, num_threads):
    ...
    for ep in range(1+epoch_restore, epochs+1):
        ...
        p_bar = None
        # Run training loop
        loses = train_online_ml(model, optimizer, data_generator_ddm_flexbound, iterations_per_epoch, 
                        batch_size, num_threads=num_threads,p_bar=p_bar, clip_value=clip_value, global_step=global_step, 
                        transform=None, n_smooth=100)
        print("LOSS: ", loses['ml_loss'])
        manager.save()
